...
├── collector/           # 采集器核心模块
│   ├── core/           # 核心组件
│   │   ├── collector_model.go      # 接口定义和常量
│   │   └── register_multi.go       # 注册管理
│   │
│   ├── adapter/        # 适配器层
│   │   ├── multi_source_adapter.go
│   │   ├── custom_multi_source_adapter.go
│   │   └── metric_labeler_injector.go
│   │
│   ├── database/       # 数据库采集器（不再细分）
│   │   ├── db_arch_status_info_collector.go
│   │   ├── db_buffer_pool_info_collector.go
│   │   ├── db_ckpttime_info_collector.go
│   │   ├── db_dual_info_collector.go
│   │   ├── db_dw_watcher_info_collector.go
│   │   ├── db_instance_log_error_info_collector.go
│   │   ├── db_instance_running_info_collector.go
│   │   ├── db_job_running_info_collector.go
│   │   ├── db_license_info_collector.go
│   │   ├── db_memory_pool_collector.go
│   │   ├── db_monitor_info_collector.go
│   │   ├── db_parameter_info_collector.go
│   │   ├── db_rapply_sys_collector.go
│   │   ├── db_rapply_time_diff_collector.go
│   │   ├── db_sessions_status_collector.go
│   │   ├── db_slow_sql_info_collector.go
│   │   ├── db_statement_type_info_collector.go
│   │   ├── db_system_info_collector.go
│   │   ├── db_tablespace_datafile_info_collector.go
│   │   ├── db_tablespace_info_collector.go
│   │   ├── db_user_list_info_collector.go
│   │   ├── db_version_info_collector.go
│   │   └── purge_collector.go
│   │
│   ├── host/           # 主机采集器
│   │   ├── exporter_host_base_info_collector.go
│   │   └── host_process_info_collector.go
│   │
│   ├── custom/         # 自定义采集器
│   │   └── custom_collector.go
│   │
│   └── build_info_collector.go  # 构建信息采集器
│
├── config/              # 配置模块（保持不变）
├── db/                  # 数据库连接管理（保持不变）
├── logger/              # 日志模块（保持不变）
├── util/                # 工具类模块（新建）
│   ├── db_util.go      # 从collector移动过来
│   └── os_util.go      # 从collector移动过来
└── dameng_exporter.go   # 主程序入口
