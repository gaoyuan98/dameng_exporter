...
enableHealthPing = true              # 是否启用周期性健康检查（禁用后仅在查询失败时确认连接状态）
...
maxOpenConns = 10                     # 最大打开连接数（maxIdleConns 已废弃，与该值一致）
...
> 💡 **降低探活压力**：将 `enableHealthPing` 设为 `false` 后，Exporter 不再按固定间隔对所有数据源执行 Ping，只在采集 SQL 报错时对该数据源补一次确认性 Ping。确认失败会立即将其降级到失败池并交由自动重试模块处理，`dmdb_up` 指标仍能即时反映真实状态。
...
dbPwd = "ENC(xxxxx)"
maxOpenConns = 20                     # 生产环境增加连接数（空闲连接自动等于该值）
queryTimeout = 10
...
| 场景 | maxOpenConns | connMaxLifetime | queryTimeout |
|------|--------------|-----------------|--------------|
| 低负载（<100 QPS） | 5 | 60分钟 | 30秒 |
| 中负载（100-500 QPS） | 15 | 30分钟 | 20秒 |
| 高负载（>500 QPS） | 30 | 10分钟 | 10秒 |
| 网络不稳定 | 10 | 5分钟 | 15秒 |
| 数据仓库 | 10 | 30分钟 | 120秒 |

> `maxIdleConns` 参数已废弃，空闲连接数会自动与 `maxOpenConns` 保持一致。
...
> 注：`--maxIdleConns` 已废弃，命令行无需再配置，空闲连接数自动与 `--maxOpenConns` 相同。
...
| --enableHealthPing | bool | true | 是否启用周期性健康检查，false 时仅在 SQL 报错时补充探活 |
...
| enableHealthPing | bool | true | 否 | 是否启用周期性健康检查（禁用后仅在SQL错误时确认） |
...
> `maxIdleConns` 字段已废弃，配置文件无需再填写，空闲连接数与 `maxOpenConns` 相同。
