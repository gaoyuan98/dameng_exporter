...
| 最大空闲连接数 | - | - | 与 `maxOpenConns` 相同 | 参数已废弃，始终等于 `maxOpenConns` | - |
| 连接最大生命周期 | `--connMaxLifetime` | `connMaxLifetime` | `30` | 连接最大生命周期（分钟） | >0 |

> **提示**：`maxIdleConns` 参数已废弃，Exporter 会自动将最大空闲连接数设置为与 `maxOpenConns` 相同的值。
...
queryTimeout = 5
maxOpenConns = 10  # maxIdleConns已废弃，空闲连接数与该值保持一致
connMaxLifetime = 30
...
dbPwd = "test_password"
queryTimeout = 5
maxOpenConns = 5  # maxIdleConns已废弃，空闲连接数与该值保持一致
connMaxLifetime = 20
...
# 无需配置 --maxIdleConns，空闲连接数会与 --maxOpenConns 相同
...
- **取值范围**：
  - `queryTimeout`: 1-300秒
- `maxOpenConns`: 1-100
- `maxIdleConns`: 已废弃，系统自动与 `maxOpenConns` 保持一致
...
#### 连接池配置
- **生产环境**：`maxOpenConns=10-20`（空闲连接自动等于该值）
- **测试环境**：`maxOpenConns=5-10`（空闲连接自动等于该值）
