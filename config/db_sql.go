...
const (
	...
	//表空间的使用率
	//意思 DSC 跟 DW不兼容
	QueryTablespaceInfoSqlStr_20260305 = `SELECT /*+DAMENG_EXPORTER*/ F.TABLESPACE_NAME,T.TOTAL_SPACE   "TOTAL_SIZE",F.FREE_SPACE   AS "FREE_SIZE"
FROM (
...
WHERE F.TABLESPACE_NAME = T.TABLESPACE_NAME`
	...
	QueryTablespaceInfoSqlStr = `SELECT /*+DAMENG_EXPORTER*/ B.TABLESPACE_NAME
       ,B.FILES
...
ORDER BY 7`
	...
	// ASM磁盘组空间信息
	QueryAsmGroupInfoSqlStr = `SELECT /*+DM_EXPORTER*/ TO_CHAR(GROUP_ID) GROUP_ID
       ,GROUP_NAME
       ,TYPE
       ,TOTAL_MB
       ,FREE_MB
 ...
 ORDER BY 6`
	...
	// 检查V$ASMGROUP视图是否存在
	QueryAsmGroupViewExistsSqlStr = "SELECT /*+DM_EXPORTER*/ COUNT(1) FROM V$DYNAMIC_TABLES WHERE NAME = 'V$ASMGROUP'"

	// 检查V$ASMGROUP视图中的TYPE字段是否存在
	QueryAsmGroupTypeColumnExistsSqlStr = "SELECT /*+DM_EXPORTER*/ COUNT(*) FROM V$DYNAMIC_TABLE_COLUMNS WHERE TABNAME = 'V$ASMGROUP' AND COLNAME = 'TYPE'"
...
`
 ...
 WHERE A.USERNAME NOT IN('SYS','SYSSSO','SYSAUDITOR');`
	...
	//新增需求 20250401
	//查询归档的所有发送状态信息
	QueryArchiveSendStatusSql = `select /*+DMDB_CHECK_FLAG*/ case when ARCH_STATUS = 'INVALID' then 0 when ARCH_STATUS = 'VALID' then 1 when ARCH_STATUS = 'ASYNC_SEND' then 2 when ARCH_STATUS is null or trim(ARCH_STATUS) = '' then -3 else -2 end ARCH_STATUS,ARCH_TYPE,ARCH_DEST,ARCH_SRC from v$arch_status`
	//归档信息LSN详情
   ...
   S.ARCH_DEST,  S.ARCH_TYPE, 
   CASE WHEN A.RPKG_LSN IS NOT NULL THEN S.MAX_SEND_LSN - A.RPKG_LSN ELSE 0 END AS LSN_DIFFERENCE, 
       LAST_SEND_CODE, 
       LAST_SEND_DESC, 
       ...
       LAST_SEND_TIME 
  FROM V$ARCH_SEND_INFO S
  LEFT JOIN (
       SELECT DEST_NAME, MAX(RPKG_LSN) AS RPKG_LSN
       FROM V$ARCH_APPLY_INFO
       GROUP BY DEST_NAME
  ) A ON S.ARCH_DEST = A.DEST_NAME;`

	QueryArchSendDetailInfoDsc = `SELECT  /*+DM_EXPORTER*/ 
   S.ARCH_DEST,  S.ARCH_TYPE, 
   CASE WHEN A.RPKG_LSN IS NOT NULL THEN S.MAX_SEND_LSN - A.RPKG_LSN ELSE 0 END AS LSN_DIFFERENCE, 
       LAST_SEND_CODE, 
       LAST_SEND_DESC, 
  ...
  FROM V$ARCH_SEND_INFO S
  LEFT JOIN (
       SELECT DEST_NAME, DSC_SEQNO, MAX(RPKG_LSN) AS RPKG_LSN
       FROM V$ARCH_APPLY_INFO
       GROUP BY DEST_NAME, DSC_SEQNO
  ) A ON S.ARCH_DEST = A.DEST_NAME
     AND A.DSC_SEQNO = (SELECT DSC_SEQNO FROM V$INSTANCE);`
  ...
  FROM (SELECT DECODE(SUBSTR(VER,1,2),'03','企业版','05','安全版','02','标准版','其他') AS BUILD_TYPE
	...
	// 检查V$LOG_HISTORY视图是否存在
	QueryRedoLogHistoryViewExistsSql = "SELECT /*+DM_EXPORTER*/ COUNT(1) FROM V$DYNAMIC_TABLES WHERE NAME = 'V$LOG_HISTORY'"
